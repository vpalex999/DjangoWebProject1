...
class bookmarks(models.Model):
    nametab=models.CharField(max_length = 30, unique = True)
    ...
    class Meta:
        ...
        verbose_name = "Разделы" # шапка таблицы при отображении строк
...
class bookmarksubmenu(models.Model):
    namesub = models.CharField(max_length=30, unique = True) # хранит название топика(подменю)
    nametabsubmenu = models.ForeignKey(bookmarks, null = True, blank = True, on_delete=models.SET_NULL) # топик может быть привязан к закладке TAB (хранит ключ выбранной закладки nametab таблицы bookmarks
